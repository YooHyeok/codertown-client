...
export default function CokkiriEdit() {
    ...
    useEffect(() => {
        ...
        // axios 호출
            ...
            .then((response)=> {
                navigate(`/cokkiri-detail/${cokkiriNo}`)
            })
            .catch((error) => {
                console.log(error);
            })
                            ...
                            <Label htmlFor='location' sm={12}>추가 파트 미리보기 (최대 5개 중복 불가)</Label>
                                ...
                                <div style={{display:'flex', width:'100%', height:'50px', margin:'0 auto', backgroundColor:'#f7f9fc', border:'var(--bs-border-width) solid var(--bs-border-color)', borderRadius:'var(--bs-border-radius)' }}>
                                    ...
                                    {cokkiri.projectParts.map((projectPart)=>{
                                        return(
                                        <div key={cokkiri.projectParts.projectPartNo} style={{display:'flex', width:'120px', height:'39px', backgroundColor:'white', margin:"5px", border:'var(--bs-border-width) solid var(--bs-border-color)', borderRadius:'var(--bs-border-radius)' , boxShadow: "5px 5px 5px rgba(0, 0, 0, 0.2)" }}>
